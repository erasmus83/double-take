...
describe('calculateOrientationCoefficient', () => {
  test('should correctly calculate pose direction for pitch=30, roll=45, yaw=60', () => {
    ...
    expected.forEach((val, i) => expect(result[i]).toBeCloseTo(val));
  });
...
describe('isFacingCamera', () => {
  it('should return false when all angles are zero (face pointing away from camera)', () => {
    expect(isFacingCamera(0, 0, 0)).toBe(false);
  });

  it('should return false when yaw is 180 degrees (Z component unchanged by yaw)', () => {
    expect(isFacingCamera(0, 0, 180)).toBe(false);
  });

  it('should return true when pitch causes Z-component to become negative and dominant', () => {
    expect(isFacingCamera(-180, 0, 0)).toBe(true);
  });

  it('should return true when pitch is -180 degrees and yaw is 180 degrees', () => {
  ...
  it('returns true when Z-component is negative and dominant', () => {
    expect(isFacingCamera(0, 180, 0)).toBe(true);
  });
  ...
  it('returns false when Z-component is negative but not dominant', () => {
    expect(isFacingCamera(0, 100, 0)).toBe(false);
  });
});
